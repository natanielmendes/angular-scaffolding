...
app.controller('DemoAppController', function($http, $location, $uibModal) {
    ...
    const apiBaseURL = "{SERVER}/api/iou/";
    let peers = [];
    let users = [];
    ...
    $http.get(apiBaseURL + "users").then((response) => users = response.data);
    ...
    demoApp.download = (iou) => {
      return $http.post('{SERVER}/download', iou, {responseType : 'arraybuffer'}).then((response) => {
...
app.controller('ModalInstanceCtrl', function ($http, $location, $uibModalInstance, $uibModal, demoApp, apiBaseURL, peers, users) {
    const modalInstance = this;

    modalInstance.peers = peers;
    modalInstance.users = users;
    modalInstance.form = {};
    ...
    modalInstance.create = () => {
        if (invalidFormInput()) {
            ...
            const createIOUEndpoint = `${apiBaseURL}create-iou?partyName=${modalInstance.form.counterparty}&iouValue=${modalInstance.form.value}&userId=${modalInstance.form.user.state.data.linearId.id}`;
